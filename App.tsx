  ...
  return (
    <div className={`h-screen bg-slate-950 flex flex-col text-slate-200 overflow-hidden font-['Inter'] ${dir}`} dir={dir}>
      ...
      <main className="flex-1 flex flex-col md:flex-row overflow-hidden">
        <div className="w-full md:w-[320px] flex flex-col p-2 gap-2 bg-slate-900/30 border-r border-white/5 shadow-2xl overflow-y-auto">
          <div className="bg-slate-900/90 rounded-2xl border border-white/10 p-2 flex flex-col gap-2">
            <div className="bg-slate-800/40 p-1.5 rounded-xl border border-white/5 flex items-center gap-1.5">
                <select value={nativeLang.code} onChange={e => setNativeLang(SUPPORTED_LANGUAGES.find(l => l.code === e.target.value)!)} className="bg-slate-900 border border-white/10 rounded-lg px-1 py-2 text-[10px] font-bold w-full text-center">{SUPPORTED_LANGUAGES.map(l => <option key={l.code} value={l.code}>{l.flag} {l.name}</option>)}</select>
          ...
          <div className="flex flex-col items-center py-1 flex-1 justify-center relative min-h-[220px]">
            <div className="scale-65 -mt-4"><Avatar state={status === ConnectionStatus.CONNECTED ? (isSpeaking ? 'speaking' : 'listening') : 'idle'} /></div>
            <button onClick={status === ConnectionStatus.CONNECTED ? stopConversation : startConversation} className={`mt-2 px-8 py-3 rounded-full font-black text-lg shadow-2xl flex items-center gap-2 active:scale-95 transition-all ${status === ConnectionStatus.CONNECTED ? 'bg-red-500' : 'bg-indigo-600 shadow-indigo-500/20'}`}>
                <Mic size={20} /> {status === ConnectionStatus.CONNECTED ? t('stop_conversation') : t('start_conversation')}
            </button>
            {(isSpeaking || status === ConnectionStatus.CONNECTED) && <div className="absolute bottom-2 w-full px-8"><AudioVisualizer isActive={true} color={isSpeaking ? "#6366f1" : "#10b981"} /></div>}
          </div>
        </div>
      </main>
    </div>
