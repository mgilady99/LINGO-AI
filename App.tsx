...
const App: React.FC = () => {
  ...
  const startConversation = async () => {
    ...
    try {
      ...
      // חיבור וקבלה של אובייקט ה-LiveSession
      ...
      scriptProcessor.onaudioprocess = (e) => {
          if (activeSessionRef.current) {
              ...
              // תיקון המבנה: שליחת אובייקט פשוט כפי שה-SDK מצפה
    ...
    <div className={`h-screen bg-slate-950 flex flex-col text-slate-200 overflow-hidden ${dir}`} dir={dir}>
      <header className="p-4 flex items-center justify-between bg-slate-900/60 border-b border-white/5">
        ...
        <div className="flex items-center gap-3">
          {userData?.role === 'ADMIN' && <button onClick={() => setView('ADMIN')} className="bg-white text-indigo-900 px-4 py-2 rounded-full font-bold text-xs flex items-center gap-2"><Settings size={14} /> {t('admin_panel')}</button>}
