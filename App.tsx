  ...
  if (!hasKey) {
    return (
      <div className="h-screen bg-slate-950 flex flex-col items-center justify-center p-8 text-center">
        <div className="w-20 h-20 bg-indigo-600 rounded-3xl flex items-center justify-center mb-8 shadow-2xl"><Key className="text-white" size={40} /></div>
        ...
        <button onClick={handleSelectKey} className="bg-indigo-600 text-white px-10 py-5 rounded-full font-black text-xl flex items-center gap-3 hover:scale-105 transition-transform shadow-xl">SELECT API KEY <ChevronRight /></button>
      </div>
    );
  }
            ...
            <div className="grid grid-cols-2 gap-2">
              {SCENARIOS.map(s => (
                <button 
                  ...
                  disabled={status !== ConnectionStatus.DISCONNECTED}
                  className={`py-3 px-2 rounded-2xl flex flex-col items-center gap-1 transition-all ${selectedScenario.id === s.id ? 'bg-indigo-600 text-white border border-indigo-400' : 'bg-slate-800/40 text-slate-500 border border-transparent'}`}
                >
            ...
            <div className="w-full flex justify-center">
              {status === ConnectionStatus.CONNECTED ? (
                <div className="flex items-center gap-4">
                  ...
                  <button onClick={stopConversation} className="bg-white text-slate-950 px-8 py-4 rounded-full font-black text-sm tracking-widest uppercase">Stop Session</button>
                </div>
              ) : (
                <button 
