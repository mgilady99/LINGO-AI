...
const App: React.FC = () => {
  ...
  const startConversation = async () => {
      ...
      (async () => {
        try {
          for await (const msg of session.listen()) {
            ...
            if (audio) {
              setIsSpeaking(true);
              const buffer = decodeAudioData(decode(audio), outputAudioContextRef.current!, 24000);
              const audioSource = outputAudioContextRef.current!.createBufferSource();
              audioSource.buffer = buffer; audioSource.connect(outputAudioContextRef.current!.destination);
              audioSource.onended = () => { if (sourcesRef.current.size === 0) setIsSpeaking(false); };
              audioSource.start(Math.max(nextStartTimeRef.current, outputAudioContextRef.current!.currentTime));
    ...
    } catch (e) { stopConversation(); alert("Check permissions and API Studio settings."); }
  };
